...
<div class="bg-white shadow rounded-lg p-6 max-w-3xl mx-auto">
    <h1 class="text-2xl font-bold mb-4">✏️ Edit Supplier</h1>
    ...
    <!-- Form -->
    <form action="{{ route('suppliers.update', $supplier->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea id="address" name="address" rows="2"
                      class="w-full border rounded px-3 py-2 focus:ring focus:border-blue-300">{{ old('address', $supplier->address) }}</textarea>
        </div>
        ...
        <div>
            ...
            <input type="number" id="total_purchase" name="total_purchase"
                   value="{{ old('total_purchase', $supplier->total_purchase) }}"
        ...
        <div>
            ...
            <textarea id="notes" name="notes" rows="2"
                      class="w-full border rounded px-3 py-2 focus:ring focus:border-blue-300">{{ old('notes', $supplier->notes) }}</textarea>
        </div>
        ...
        <div>
            ...
            @if ($supplier->image)
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $supplier->image) }}" alt="Gambar" class="h-32 rounded border">
                </div>
            @endif
