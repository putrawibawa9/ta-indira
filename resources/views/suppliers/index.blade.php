...
<div class="bg-white shadow rounded-lg p-6">
    ...
    <div class="flex items-center justify-between mb-4">
        <div>
            ...
            <p class="text-gray-500 text-sm">Data supplier yang terdaftar di sistem</p>
        </div>
            ...
            <tbody>
                @forelse ($suppliers as $index => $supplier)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-2 border text-center">{{ $suppliers->firstItem() + $index }}</td>
                        <td class="px-4 py-2 border font-semibold">{{ $supplier->name }}</td>
                        ...
                        <td class="px-4 py-2 border">
                            Rp {{ number_format($supplier->total_purchase, 0, ',', '.') }}
                        </td>
                        <td class="px-4 py-2 border">{{ $supplier->notes ?? '-' }}</td>
                        <td class="px-4 py-2 border text-center">
                            @if ($supplier->image)
                                <img src="{{ asset('storage/' . $supplier->image) }}"
                                     alt="Gambar" class="h-12 w-12 object-cover rounded">
                            @else
