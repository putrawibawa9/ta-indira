...
<div class="max-w-lg mx-auto bg-white p-6 rounded shadow">
    ...
    <form action="{{ route('transactions.update', $transaction) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="total" class="form-control @error('total') is-invalid @enderror" value="{{ old('total', $transaction->total) }}" min="0" step="0.01" required>
            @error('total')<div class="invalid-feedback">{{ $message }}</div>@enderror
        </div>
        <div class="mb-3">
            <label for="notes" class="form-label">Catatan</label>
            <textarea name="notes" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $transaction->notes) }}</textarea>
